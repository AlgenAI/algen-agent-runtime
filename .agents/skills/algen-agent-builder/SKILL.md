...
a Runtime `WorkflowManifest` for coordination. Read `references/project-shape.md`; for a workflow,
also read `references/workflows.md`. Use `$algen-agent-connector` when external tools, databases,
retrieval, MCP, or remote APIs are part of the request.
...
Studio-specific workflow dialect.
